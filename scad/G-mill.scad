...
module I (l, w1, w2) {
	...
	translate([-L_s, -(w1/2 - pipe_w/2), pipe_w+L_s])
	rotate([0, 180, -90])
	L(w1);

	translate([l + L_s, w2/2 + pipe_w/2, pipe_w+L_s])
	rotate([0, 180, 90])
	L(w2);
}
...
module base() {
	...
	translate([base_length + L_s, (operation_area_y + pipe_w)/2, pipe_w+L_s])
	rotate([0, 180, 90])
	L(operation_area_y);
}
